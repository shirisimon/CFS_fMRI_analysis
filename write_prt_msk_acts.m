...
files_pattern = '*_exp3*.csv';
subjects_dir_depth = '1';
files_dir_depth = '2';

% params 2 prt
output_file_ext = 'msk3';
prt_type = 'vol';
convert2Vol = 1;
tr_length = 2;
tr_num = 398;
skip_tr = 2;
lag = 0;
predictors_names = {'msk_low_act1', 'msk_high_act1', ...
    'msk_low_act2', 'msk_high_act2', ...
    'msk_low_act3', 'msk_high_act3', ...
    'report'};
predictors_idx = [1 3 4]; % the importent colomns in logfile to prt
...
for t = 1:length(target_predictors)
    for m = 1:length(opacity_perdictors)
        c=c+1;
        data(c).name = name{c};
        data(c).data(:,1) = varibls(varibls(:,1) == target_predictors(t) & ...
            varibls(:,3) == opacity_perdictors(m), 5);
        data(c).data(:,2) = data(c).data(:,1);
    end
end
data(c+1).name = 'report';
data(c+1).data(:,1) = varibls(varibls(:,4) == 1, 8);
